...
class PlayersInputContainer extends JPanel {
    ...
    void setNumberOfPlayers(int numberOfPlayers) {
        ...
        if (delta > 0) {
            for (int i = 0; i < delta; i++) {
                ...
                playerContainerToAdd.setOnFocusLost((String playerName) -> {
                    boolean isUnique = PlayersInputContainer.this.verifyPlayerNameUnique(playerName);
                    if (isUnique) {
                ...
                playerContainerToAdd.setOnChanged((String value) -> {
                    System.out.printf("Player input container updated with text %s\n", value);
                    System.out.printf("All player names are unique ? %b\n", PlayersInputContainer.this.verifyAllPlayerNamesAreUnique());
                    if (PlayersInputContainer.this.onPlayersUpdated == null) {
                        return;
                    }
                    PlayersInputContainer.this.onPlayersUpdated.accept(PlayersInputContainer.this.arePlayersValid());
                });
    ...
    private static class PlayerInputContainer extends JPanel {
            ...
            this.usernameField.addFocusListener(new FocusListener() {
                ...
                public void focusLost(FocusEvent e) {
                    if (PlayerInputContainer.this.onFocusLost != null) {
                        PlayerInputContainer.this.onFocusLost.accept(PlayerInputContainer.this.usernameField.getText());
                    }
                }
            });
            this.usernameField.addCaretListener((CaretEvent e) -> {
                PlayerInputContainer.this.onChanged.accept(PlayerInputContainer.this.usernameField.getText());
            });
        ...
        boolean isPlayerValid() {
            System.out.printf("Is player valid function called. Is error message is visible ? %b\n Is username field valid ? %b\n",
                this.errorMessageLabel.isVisible(),
                this.usernameField.getText().matches("^[a-zA-Z0-9]{4,10}$"));
            return !this.errorMessageLabel.isVisible() && this.usernameField.getText().matches("^[a-zA-Z0-9]{4,10}$");
        }
