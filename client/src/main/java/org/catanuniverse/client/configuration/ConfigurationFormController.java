...
class ConfigurationFormController {
  ...
  public ConfigurationFormController(ConfigurationForm configurationForm, Consumer<GameSettings> onSaved) {
  ...
  void gameSettingsPaneListener(GameSettings settings) {
    ...
    if (settings instanceof LocalGameSettings) {
      this.configurationForm.getPlayersInputContainer().setNumberOfPlayers(
          settings.getNumberOfRequestedPlayers()
      );
    }
  ...
  private void updateSettings(GameType type) {
    ...
    this.settings = switch (type) {
      case LOCAL -> new LocalGameSettings(GameSettings.DEFAULT_CAPACITY, GameSettings.DEFAULT_NUMBER_OF_AI);
      case MP_GUEST -> new MultiPlayerGuestGameSettings();
      case MP_HOST -> new MultiPlayerHostGameSettings(GameSettings.DEFAULT_CAPACITY, GameSettings.DEFAULT_NUMBER_OF_AI);
    };
    if (oldSettings != null) {
  ...
  private void updatePlayersInputContainer() {
    this.configurationForm.getPlayersInputContainer().setNumberOfPlayers(this.settings.getNumberOfRequestedPlayers());
